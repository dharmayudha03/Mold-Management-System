    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
            <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between">
                <h6 class="m-0 font-weight-extrabold text-gray-900"><i class="fas fa-edit text-primary mr-2"></i>Form Edit Master Rak & No. Rak</h6>
            ...
            <div class="card-body p-4">
                <form action="{{ route('list-raks.update', $item->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="rak" id="rak_select" required class="form-select text-xs">
                            ...
                            @foreach($existingRaks as $er)
                                <option value="{{ $er }}" {{ old('rak', $item->listRak->rak ?? '') == $er ? 'selected' : '' }}>{{ $er }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="norak" value="{{ old('norak', $item->norak) }}" required class="form-control text-xs py-2 uppercase" style="border-radius: 0.75rem;">
                    </div>
