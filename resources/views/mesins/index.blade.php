<x-app-layout>
    ...
    <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
        <div class="card-body p-3.5">
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                ...
                <!-- Filter Form -->
                <form method="GET" action="{{ route('mesins.index') }}" class="d-flex flex-wrap align-items-center gap-2">
                    <div class="position-relative" style="width: 280px;">
                        ...
                        <input type="text" name="search" value="{{ $search }}" placeholder="Cari No Mesin / Nama / Class..." 
                    ...
                    @if($search || $status)
                        <a href="{{ route('mesins.index') }}" class="btn btn-sm btn-outline-secondary font-weight-bold px-3 py-2" style="border-radius: 0.75rem;">Reset</a>
                    @endif
                </form>

                <!-- Create Button -->
                <div class="shrink-0">
                    <a href="{{ route('mesins.create') }}" class="btn btn-sm btn-primary font-weight-bold px-3.5 py-2" style="background-color: #2563eb; border: none; border-radius: 0.75rem;">
    ...
    <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
        <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between">
            ...
            <span class="badge bg-light text-gray-700 border px-3 py-1.5 font-weight-bold" style="border-radius: 50rem;">Total: {{ $mesins->total() }} Records</span>
        </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-light text-gray-800 border px-2.5 py-1 font-weight-bold" style="border-radius: 0.5rem;">
                                        {{ $item->classMesin->class ?? '-' }}
                                    </span>
                                </td>
                                <td class="font-weight-semibold text-gray-700">{{ $item->posisi }}</td>
                                <td>
                                ...
                                <td class="text-right pr-4">
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 py-1" style="border-radius: 0.75rem; font-size: 0.8rem;">
                                            <li>
                                                <a href="{{ route('mesins.edit', $item->id) }}" class="dropdown-item py-1.5 px-3 text-gray-700 font-weight-bold">
                                                    <i class="fas fa-edit text-primary mr-2"></i> Edit
                                                </a>
                                            </li>
                                            <li><hr class="dropdown-divider my-1"></li>
                                            <li>
                                                <form action="{{ route('mesins.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Mesin ini?')">
                                                    @csrf
                                                    @method('DELETE')
