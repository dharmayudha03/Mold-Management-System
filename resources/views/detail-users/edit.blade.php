    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
            <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between">
                <h6 class="m-0 font-weight-extrabold text-gray-900"><i class="fas fa-edit text-primary mr-2"></i>Form Edit Detail User (PIC Petugas)</h6>
                <span class="badge bg-light text-gray-700 border px-3 py-1 font-weight-bold" style="border-radius: 50rem;">Update Detail PIC</span>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('detail-users.update', $detailUser->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="role_id" required class="form-select text-xs py-2" style="border-radius: 0.75rem;">
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}" {{ old('role_id', $detailUser->role_id) == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ old('name', $detailUser->name) }}" required 
                    ...
                    <div class="pt-3 border-top d-flex align-items-center justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-sm btn-primary font-weight-bold px-4 py-2" style="border-radius: 0.75rem; background-color: #2563eb; border: none;">
                            <i class="fas fa-save mr-1.5"></i> Update Detail PIC
                        </button>
                    </div>
                </form>
            </div>
