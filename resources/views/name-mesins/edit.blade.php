        ...
        <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
            <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between">
                <h6 class="m-0 font-weight-extrabold text-gray-900">
                    <i class="fas fa-edit text-primary mr-2"></i>Form Edit Nama Mesin
                </h6>
            ...
            <div class="card-body p-4">
                <form action="{{ route('name-mesins.update', $nameMesin->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="form-group-item">
                            ...
                            <select name="list_mesin_id" id="list_mesin_id" required class="form-select">
                                ...
                                @foreach($listMesins as $lm)
                                    <option value="{{ $lm->id }}" {{ old('list_mesin_id', $nameMesin->list_mesin_id) == $lm->id ? 'selected' : '' }}>{{ $lm->code }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-grid-1">
                        <div class="form-group-item">
                            ...
                            <input type="text" name="name" value="{{ old('name', $nameMesin->name) }}" required
                    ...
                    <div class="pt-4 border-top d-flex align-items-center justify-content-end gap-2 mt-4">
                        ...
                        <button type="submit" class="btn btn-sm btn-primary font-weight-bold px-4 py-2" style="border-radius: 0.75rem; background-color: #2563eb; border: none;">
                            <i class="fas fa-save mr-1.5"></i> Update Nama Mesin
                        </button>
                    </div>
                </form>
            </div>
