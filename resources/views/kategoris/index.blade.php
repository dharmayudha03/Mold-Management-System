<x-app-layout>
    ...
    <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
        <div class="card-body p-3.5">
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                <form method="GET" action="{{ route('kategoris.index') }}" class="d-flex align-items-center gap-2">
                    ...
                    @if($search)
                        <a href="{{ route('kategoris.index') }}" class="btn btn-sm btn-outline-secondary font-weight-bold px-3 py-2" style="border-radius: 0.75rem;">Reset</a>
                    @endif
                </form>

                <div class="shrink-0">
                    <a href="{{ route('kategoris.create') }}" class="btn btn-sm btn-primary font-weight-bold px-3.5 py-2" style="background-color: #2563eb; border: none; border-radius: 0.75rem;">
    ...
    <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
        <div class="card-header bg-white py-3 border-bottom d-flex align-items-center justify-content-between">
            ...
            <span class="badge bg-light text-gray-700 border px-3 py-1.5 font-weight-bold" style="border-radius: 50rem;">Total: {{ $kategoris->total() }} Records</span>
        </div>
        <div class="card-body p-0">
                                ...
                                <td class="text-right pr-4">
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 py-1" style="border-radius: 0.75rem; font-size: 0.8rem;">
                                            <li>
                                                <a href="{{ route('kategoris.edit', $item->id) }}" class="dropdown-item py-1.5 px-3 text-gray-700 font-weight-bold">
                                                    <i class="fas fa-edit text-primary mr-2"></i> Edit
                                                </a>
                                            </li>
                                            <li><hr class="dropdown-divider my-1"></li>
                                            <li>
                                                <form action="{{ route('kategoris.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus Kategori ini?')">
                                                    @csrf @method('DELETE')
            ...
            <div class="p-3 border-top bg-white">{{ $kategoris->links() }}</div>
        </div>
    </div>
</x-app-layout>
