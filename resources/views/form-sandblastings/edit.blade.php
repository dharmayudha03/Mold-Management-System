            ...
            <div class="card-body p-4">
                <form action="{{ route('form-sandblastings.update', $formSandblasting->id) }}" method="POST">
                        ...
                        <div class="form-group-item">
                            ...
                            <select name="role_id" id="role_id" required class="form-select">
                                ...
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ $formSandblasting->role_id == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group-item">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="keterangan" rows="2" class="form-control text-xs" style="border-radius: 0.75rem;" placeholder="Tambahkan keterangan/catatan tambahan jika ada...">{{ old('keterangan', $formSandblasting->keterangan) }}</textarea>
                    </div>
