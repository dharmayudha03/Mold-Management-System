    ...
    <div class="card shadow-sm border-0 mb-4" style="border-radius: 1rem;">
        <div class="card-body p-3.5">
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                <form method="GET" action="{{ route('cetakan-naiks.index') }}" class="d-flex align-items-center gap-2">
                    <div class="position-relative" style="width: 280px;">
                        ...
                        <input type="text" name="search" value="{{ $search }}" placeholder="Cari Code Item / Mesin / Ket..." 
                    ...
                    @if($search)
                        <a href="{{ route('cetakan-naiks.index') }}" class="btn btn-sm btn-outline-secondary font-weight-bold px-3 py-2" style="border-radius: 0.75rem;">Reset</a>
                    @endif
                </form>

                @if(!auth()->user()->hasRole('User'))
                <div class="shrink-0">
                    <a href="{{ route('cetakan-naiks.create') }}" class="btn btn-sm btn-primary font-weight-bold px-3.5 py-2" style="background-color: #2563eb; border: none; border-radius: 0.75rem;">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0 text-xs">
                    ...
                    <tbody>
                        @forelse($cetakanNaiks as $item)
                            <tr>
                                <td class="text-gray-700 font-weight-bold">{{ \Carbon\Carbon::parse($item->tanggalnaik)->format('d/m/Y') }}</td>
                                ...
                                <td class="font-weight-semibold text-gray-800">{{ $item->keterangan }}</td>
                                    ...
                                    @if(!auth()->user()->hasRole('User'))
                                        <div class="dropdown d-inline-block">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0 py-1" style="border-radius: 0.75rem; font-size: 0.8rem;">
                                                <li>
                                                    <a href="{{ route('cetakan-naiks.edit', $item->id) }}" class="dropdown-item py-1.5 px-3 text-gray-700 font-weight-bold">
                                                        <i class="fas fa-edit text-primary mr-2"></i> Edit
                                                    </a>
                                                </li>
                                                @if(!auth()->user()->hasRole('Leader') && !auth()->user()->hasRole('leader'))
                                                <li><hr class="dropdown-divider my-1"></li>
                                                <li>
                                                    <form action="{{ route('cetakan-naiks.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus record Cetakan Naik ini?')">
                                                        @csrf @method('DELETE')
            ...
            <div class="p-3 border-top bg-white">{{ $cetakanNaiks->links() }}</div>
        </div>
    </div>
</x-app-layout>
